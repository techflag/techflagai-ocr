...
function MainCanvasMouseDown(e) {
    isMouseDown = true;
    pALastX = e.pageX - MainCanvas.offsetLeft -20;
    pALastY = e.pageY - MainCanvas.offsetTop -100;
    if (MainCamCutMode == 2) {
        pACurrentX = pALastX;
        pACurrentY = pALastY;
    }
...
function MainCanvasMouseMove(e) {
    if (isMouseDown) {
        ...
        if (MainCamCutMode == 2) {
            ...
            pACurrentX = e.pageX - MainCanvas.offsetLeft -20;
            pACurrentY = e.pageY - MainCanvas.offsetTop -100;
...
function CaptureZpPhoto(leftFilePath, rightFilePath) {

    if (isSocketConnect) {

        var leftpath = encodeURI(leftFilePath);
        var leftpathArray = stringToByte(leftpath);

        var rightPath = encodeURI(rightFilePath);
        var rightPathArray = stringToByte(rightPath);

        var len = leftpathArray.length + rightPathArray.length;
        var pathDataArray = new Uint8Array(len);
        for (var i = 0; i < leftpathArray.length; i++) {
            pathDataArray[i] = leftpathArray[i];
        }
        for (var i = 0; i < rightPathArray.length; i++) {
            pathDataArray[leftpathArray.length + i] = rightPathArray[i];
        }
        ...
        while (tmpLen > 0) {
            ...
            if (tmpLen > 0) {
                ...
                aDataArray[4] = len >> 8 & 0xff;
                aDataArray[5] = len & 0xff;
                aDataArray[6] = leftpathArray.length >> 16 & 0xff;
                aDataArray[7] = leftpathArray.length >> 8 & 0xff;
                aDataArray[8] = leftpathArray.length & 0xff;
                aDataArray[9] = rightPathArray.length >> 16 & 0xff;
                aDataArray[10] = rightPathArray.length >> 8 & 0xff;
                aDataArray[11] = rightPathArray.length & 0xff;
                aDataArray[12] = packageCount >> 8 & 0xff;   //包总数
                aDataArray[13] = packageCount & 0xff;   //包总数
                aDataArray[14] = 90;   //分包长度
                aDataArray[15] = pindex >> 8 & 0xff;   //包序号
...
function RZCompare(Text1Path, Text2Path) {

    if (isSocketConnect) {

        var text1path = encodeURI(Text1Path);
        var text1pathArray = stringToByte(text1path);

        var text2path = encodeURI(Text2Path);
        var text2pathArray = stringToByte(text2path);

        var len = text1pathArray.length + text2pathArray.length;
        var pathDataArray = new Uint8Array(len);
        for (var i = 0; i < text1pathArray.length; i++) {
            pathDataArray[i] = text1pathArray[i];
        }
        for (var i = 0; i < text2pathArray.length; i++) {
            pathDataArray[text1pathArray.length + i] = text2pathArray[i];
        }
        ...
        while (tmpLen > 0) {
            ...
            if (tmpLen > 0) {
                ...
                aDataArray[4] = len >> 8 & 0xff;
                aDataArray[5] = len & 0xff;
                aDataArray[6] = text1pathArray.length >> 16 & 0xff;
                aDataArray[7] = text1pathArray.length >> 8 & 0xff;
                aDataArray[8] = text1pathArray.length & 0xff;
                aDataArray[9] = text2pathArray.length >> 16 & 0xff;
                aDataArray[10] = text2pathArray.length >> 8 & 0xff;
                aDataArray[11] = text2pathArray.length & 0xff;
                aDataArray[12] = packageCount >> 8 & 0xff;   //包总数
                aDataArray[13] = packageCount & 0xff;   //包总数
                aDataArray[14] = 90;   //分包长度
                aDataArray[15] = pindex >> 8 & 0xff;   //包序号
        ...
        socket.onmessage = function (event) {
            ...
            if (rDataArr.length > 0) {
                ...
                if (rDataArr[0] == 0x55 && rDataArr[1] == 0x66) {
                    ...
                    if (rDataArr[2] == 0xE3) {
                        var flag = rDataArr[4];
                        GetZhanpingResultCB(flag);//提示是否展平成功
                     }

                    //人证比对
                    if (rDataArr[2] == 0xE4) {
                        var flag = rDataArr[3];
                        GetRZResultCB(flag); //提示是否展平成功
                    }
